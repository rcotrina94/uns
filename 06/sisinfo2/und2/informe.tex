...
\chapter{Modelo de Negocio}
  La empresa Botica Patricia que solicitó el proyecto de desarrollo software consta de una área de atención al cliente, una área de ventas, una área de recursos, un almacén central y una área de administración y control general.
  ...
  \PNG[0.65]{biz_model}{Diagrama de representación de subsistemas del negocio}{diagram:biz_model}
  
  \PNG[0.4]{usecase_biz}{Modelo de casos de uso general de Negocio}{diagram:usecase_biz}
  
  \section{Modelo de Casos de Uso}
  La empresa interactúa con distintos elementos externos, entre los que se identifican el cliente externo (persona o entidad que solicita la compra de productos a la empresa), el proveedor (persona o entidad que reabastece de productos a la empresa) y por último el vendedor, que es una persona encargada de servir los pedidos desde el punto de venta a los clientes, y el administrador que se encarga de asegurarse de que los indicadores de la empresa sean los adecuados.
  
  \PNG[0.35]{usecase_general}{Modelo de casos de uso general}{diagram:usecase_general}
  
  \PNG[0.75]{usecase_admin_explode}{Modelo de casos de uso del Administrador}{diagram:usecase_admin_explode}
  
  \PNG[0.79]{usecase_final}{Casos de uso de la aplicación}{diagram:usecase_final}{http://yuml.me/edit/deba96b9}
  
  \section{Modelo de Objetos}
    \subsection{Modelo de objetos de Administrar Producto}
      \PNG[0.69]{objectmodel_adm_product}{Modelo de objetos de Administrar Producto}{diagram:objectmodel_adm_product}
      
      
    \subsection{Modelo de objetos de Administrar Proveedores}
      \PNG[0.69]{objectmodel_adm_provider}{Modelo de objetos de Administrar Producto}{diagram:objectmodel_adm_provider}
  
  
    \subsection{Modelo de objetos de Listar Productos}
      \PNG[0.69]{objectmodel_product_listing}{Modelo de objetos de Listar Productos}{diagram:objectmodel_product_listing}{http://yuml.me/edit/316d35d1}
      
      
    \subsection{Modelo de objetos de Consultar Producto}
      \PNG[0.69]{objectmodel_product_detail}{Modelo de objetos de Consultar Producto}{diagram:objectmodel_product_detail}{http://yuml.me/edit/5b604ab9}
      
    \subsection{Modelo de objetos de Vender Producto}
      \PNG[0.69]{objectmodel_product_sale}{Modelo de objetos de Vender Producto}{diagram:objectmodel_product_sale}{http://yuml.me/edit/29281812}
      
                              
\part{Fase de elaboración}
\chapter{Modelado de Sistema}
...
\subsection{D.C.U. Registrar Venta}
  ...
  \textbf{ACTORES:} & Vendedor y Sistema \\\hline
  \textbf{DESCRIPCIÓN:} & Luego de la generación de tickets el vendedor debe proceder a guardar en el Sistema el registro de esa venta.\\\hline
  \textbf{FLUJO DE EVENTOS}  \\
  1.- & El vendedor luego de imprimir el ticket al cliente, pulsará la opción para Guardar Venta.\\
  2.- & Las casillas automaticamente se guardarán con los datos previamente llenados \\\hline
  \end{tabularx}
  \caption{Descripción de Caso de Uso - Registrar Venta}
  \end{table}

\subsection{D.C.U. Imprimir comprobante}

  {\renewcommand{\arraystretch}{1.25}%
  \begin{table}[H]
  \begin{tabularx}{\textwidth}{r|X}
  \textbf{ID}: & CU08\\\hline
  \textbf{NOMBRE}: & IMPRIMIR COMPROBANTE\\\hline
  \textbf{ACTORES:} & Cajero y Sistema \\\hline
  \textbf{DESCRIPCIÓN:} & El sistema genera un comprobante de pago a partir del ticket, y se imprimirá luego de cancelado el pago. \\\hline
  \textbf{FLUJO DE EVENTOS}  \\
  1.- & Luego de cancelado el pago el cajero solicitará al Sistema una muestra del comprobante de pago.\\
  2.- & El cajero procederá a la impresion del comprobante.\\
  3.- & El cajero le entregará el comprobante al cliente\\
  4.- & El sistema guardará automaticamente una copia de cada factura generada.\\\hline
  \end{tabularx}
  \caption{Descripción de Caso de Uso - Imprimir Comprobante}
  \end{table}
...
\subsection{D.C.U. Registrar Compras}
  ...
  \begin{tabularx}{\textwidth}{r|X}
  \textbf{ID}: & CU09\\\hline
  \textbf{NOMBRE}: & REGISTRAR PAGO\\\hline
  \textbf{ACTORES:} & Cajero y Sistema \\\hline
  \textbf{DESCRIPCIÓN:} & El sistema automaticamente guardará una copia del comprobante de pago. A partir de este generará un registro de Pago.  \\\hline
  \textbf{FLUJO DE EVENTOS}  \\
  1.- & El sistema luego de emitir el comprobante autoguardará los datos de la factura\\
  2.- & El Sistema verificará que no falte ningún dato.\\
  3.- & De faltar algún dato el sistema lo reportará, para que así sea el cajero quien se encargue de autocompletarlo.\\
  4.- & Luego el sistema procederá a guardar el registro.
  \end{tabularx}
  \caption{Descripción de Caso de Uso - Registrar Compras}
  \end{table}
\newpage
\chapter{Análisis y diseño}
  ...
  \PNG[0.69]{usecase_system_general}{Casos de uso de la aplicación}{diagram:usecase_system_general}{http://yuml.me/edit/b08ffb83}
  
  \section{Identificación de paquetes según casos de uso}
  \PNG[0.7]{packet_by_use_case}{Identificación de paquetes según casos de uso}{diagram:packet_by_use_case}
  
  \section{Identificación de paquetes según clases de dominio}
  \PNG[0.9]{packet_by_domain}{Identificación de paquetes según casos de uso}{diagram:packet_by_domain}
  
  \section{Diagrama de sequencia: Registrar Producto}
  \PNG[0.7]{sequence_prod}{Diagrama de sequencia: Registrar Producto}{diagram:sequence_prod}
  
  \section{Diagrama de sequencia: Registrar Proveedor}
  \PNG[0.7]{sequence_register_buy}{Diagrama de sequencia: Registrar Proveedor}{diagram:sequence_register_buy}
  
  \section{Diagrama de sequencia: Registrar Compra}
  \PNG[0.7]{sequence_register_provider}{Diagrama de sequencia: Registrar Compra}{diagram:sequence_register_provider}
  
  \section{Diagrama de sequencia: Consultar Reporte}
  \PNG[0.7]{sequence_report_consult}{Diagrama de sequencia: Consultar Reporte}{diagram:sequence_report_consult}
  
  \section{Modelo de despliegue}
  \PNG[0.7]{deployment}{Diagrama de despliegue}{diagram:deployment}
  

\part{Fase de Construcción}

  \chapter{Modelado de Base de Datos}
  ...
  \PNG[0.67]{db_model_final}{Diagrama de Entidad-Relación de la aplicación}{diagram:database_final}{}
